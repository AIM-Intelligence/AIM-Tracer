...
export function TraceGraphRagView({
  ...
  useEffect(() => {
    const newActiveNodeLog = trace.output.active_paths.active_node_log
      ? trace.output.active_paths.active_node_log
      : JSON.parse(trace.output.active_paths).active_node_log
        ? JSON.parse(trace.output.active_paths).active_node_log
        : defaultActiveNodeLog;
    setActiveNodeLog(traceId, newActiveNodeLog);
  }, [
    trace.output?.active_paths?.active_node_log,
    setActiveNodeLog,
    traceId,
    trace.output?.active_paths,
  ]);
  ...
  const graphData = generateGraph(data, parents);
